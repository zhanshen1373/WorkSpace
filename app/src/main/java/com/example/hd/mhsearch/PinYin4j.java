...
public class PinYin4j {
     ...
     * 获取拼音集合
     */
    public Set<String> getPinyin(boolean qporjp, String src) {
        char[] srcChar;
        srcChar = src.toCharArray();


        Boolean loadwhat = hm.get("loadwhat");
        hm.put("loadwhat", qporjp);

        if (loadwhat==null){
            PinyinHelper.getInstance(qporjp);
        }else if (loadwhat!=qporjp )
        PinyinHelper.getInstance(qporjp);
        // 1:多少个汉字
        // 2:每个汉字多少种读音
